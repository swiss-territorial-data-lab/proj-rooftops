...
if __name__ == "__main__":
    ...
    for row in join_tiles_roofs.itertuples():
        ...
        id_list.append(f'({round(bounds[0])}, {round(bounds[1])}, {egid})')
        coords = misc.bbox(bounds)
        coords_list.append(coords)


    bbox_gdf = gpd.GeoDataFrame(pd.DataFrame({'id': id_list, 'EGID': egid_list}), crs='epsg:2056', geometry=coords_list).drop_duplicates(subset='EGID')
    feature_path = os.path.join(OUTPUT_DIR, 'bbox.gpkg')
    bbox_gdf.to_file(feature_path)
    written_files.append(feature_path)  
    ...
    for egid in tqdm(unique_egid, desc="Production of tiles fitting the extent of the roof", total=len(unique_egid)):
        ...
        if os.path.exists(feature_path) & (not OVERWRITE):
            continue
        ...
        if len(tiles_list) > 1:
            ...
            mosaic, output_affine = merge(raster_to_mosaic)

            with rasterio.open(tiles_list[0]) as src:
                output_meta = src.meta.copy()
            output_meta.update(
            {"driver": "GTiff",
                "height": mosaic.shape[1],
                "width": mosaic.shape[2],
                "transform": output_affine
            })
        ...
        bbox_shape = bbox_gdf.loc[bbox_gdf['EGID'] == egid, 'geometry'].buffer(BUFFER, join_style=2)  
