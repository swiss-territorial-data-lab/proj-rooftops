...
parser = argparse.ArgumentParser(description="The script tests segmentation functions to detect the objects on potentially free roofs based on the intensity raster.")
...
SAVE_NORMALIZED_INTESITY = cfg['save_normalized_intensity']
...
for roof_id in tqdm(tiles_per_roof['OBJECTID'].unique().tolist(), desc='Segmenting roof plans...'):
    ...
    for roof_and_tile in needed_tiles.itertuples():
        ...
        if METHOD == 'felzenszwalb':
            ...
            logger.error('No method corresponds to the chosen one.')
            sys.exit(1)

        im_profile.update(transform=mask_transform, crs='EPSG:2056', width=segm_mask.shape[1], height=segm_mask.shape[0], nodata=0)
        with rasterio.open(os.path.join(OUTPUT_PATH, str(roof_and_tile.OBJECTID) + '_' + LAYER + '.tif'), 'w', **im_profile) as dst:
